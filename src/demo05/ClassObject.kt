...
//	可以像使用普通函数那样使用构造函数创建类实例：
	val site = ClassPer() // Kotlin 中没有 new 关键字
	println("使用构造函数创建类实例,$site")

//	要使用一个属性，只要用名称引用它即可
//	site.name           // 使用 . 号来引用
//	site.url
	println("使用属性 ${site.name} and ${site.url}")
...
//	如果构造器有注解，或者有可见度修饰符，这时constructor关键字是必须的，注解和修饰符要放在它之前。
	println(People("danny", "zhou"))
	println(People("danny", "zhou").printName())
	println("=======================次构造函数======================")
	//	类也可以有二级构造函数，需要加前缀 constructor:
	class Person3 {
		constructor(parent: Person) {
			println("次构造函数 ${parent.lastName}")
		}
	}
...
//  在同一个类中代理另一个构造函数使用 this 关键字：
	class Person4(val name: String) {
		constructor (name: String, age: Int) : this(name) {
			// 初始化...
			println("次构造函数代理主构造函数 \n$name , $age")
		}
	}
	println(Person4("danny", 22))
...
//	内部类会带有一个对外部类的对象的引用，所以内部类可以访问外部类成员属性和成员函数。
	...
	println(demo)
	val demoOuter2 = Outer2().Inner().innerTest()
...
//	public     // 所有调用的地方都可见
//	internal   // 同一个模块中可见
	println("classModifier: 类属性修饰符，标示类本身特性。")
	println("abstract    // 抽象类  ")
	println("final       // 类不可继承，默认属性")
	println("enum        // 枚举类")
	println("open        // 类可继承，类默认是final的")
	println("annotation  // 注解类")
	println("accessModifier: 访问权限修饰符")
	println("private    // 仅在同一个文件中可见")
	println("protected  // 同一个文件中或子类可见")
	println("public     // 所有调用的地方都可见")
	println("internal   // 同一个模块中可见")
}
